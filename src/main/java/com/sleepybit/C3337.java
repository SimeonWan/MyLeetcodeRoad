...
public class C3337 {
    ...
    public static int lengthAfterTransformations(String s, int t, List<Integer> nums) {
        for (int i = 0; i < t; i++) {
            s = transStringFormat(s, nums);
        }
        return s.length();
    }

    public static String transStringFormat(String s, List<Integer> nums) {
        StringBuilder stringBuilder = new StringBuilder();
        for (byte aByte : s.getBytes()) {
            for (int j = 1; j <= nums.get(aByte - 'a'); j++) {
                if (aByte + j > 'z') {
                    stringBuilder.append((char) (aByte + j - 'z' + 'a' - 1));
                } else {
                    stringBuilder.append((char) (aByte + j));
                }
            }
        }
